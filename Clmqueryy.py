...
# Pinecone setup
pc = Pinecone(api_key="pcsk_3KW6iE_KnanUyv8KgBiSzePuUuWEdyBV3NEzYeR7LruMRspisetp4UtTHFxKiV5Dn4T4tm")
index_name = "clm"
...
# LLM setup
os.environ["GOOGLE_API_KEY"]="AIzaSyB5SqyUhL5GSSjrgLj1cegI9GxZ-51gSKM"
...
prompt1 = PromptTemplate(
    ...
    template="""
        ...
        Your goal is to identify specific client names (JPMC, Zurich, Pfizer) and document types (MSA, SoW, CR) from the user input.

        User Query: {topic}

        If a client name (JPMC, Zurich) is found, return it as 'client_name'. 
    ...
    for key, value in data.items():
        ...
        if isinstance(value, str):
            ...
            if value != 'no':
                ...
                if key == "Client_name" and value in ["JPMC", "Zurich", "Pfizer"]:
                    filtered_data[key] = value.capitalize()
                elif key == "Document_type" and value in ["MSA", "SoW"]:
